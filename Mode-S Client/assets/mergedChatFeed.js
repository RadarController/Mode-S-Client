...
(function () {
    ...
    async function tick(cfg) {
        const {
            ...
            // ✅ NEW: allow chat overlays to turn off EventSub merging
            includeEvents = true
        } = cfg;

        try {
            // If events are disabled, don't fetch them at all.
            const [chatJson, eventsJson] = await Promise.all([
                safeCall(fetchChat, null),
                includeEvents ? safeCall(fetchEvents, null) : Promise.resolve(null)
            ]);

            const chatItemsRaw = extractChatItems ? (extractChatItems(chatJson) || []) : [];
            const eventsRaw =
                includeEvents && eventsJson
                    ? ((eventsJson.events) ? eventsJson.events : eventsJson)
                    : [];
            ...
            const eventIndex = includeEvents ? buildEventIndex(eventNorms) : new Map();
            ...
            for (const raw of chatItemsRaw) {
                ...
                if (includeEvents && isChatDuplicateOfEvent(n, eventIndex, dedupeWindowMs)) continue;
                merged.push(n);
            }
