...
public:
    ...
    // Interactive OAuth (one-time) to obtain a refresh token with required scopes (e.g. chat:read/chat:write).
    // redirect_uri must exactly match a Redirect URL configured in the Twitch Developer Console for your app.
    // Example: http://localhost:17845/auth/twitch/callback
    ...
    // On success, exchanges code -> tokens, validates, persists to config, updates current_ snapshot.
    // Handle the OAuth callback with an authorization code.
    // If state is provided, it is validated against the most recent state generated by BuildAuthorizeUrl().
    bool HandleOAuthCallback(const std::string& code,
                             const std::string& redirect_uri,
                             std::string* out_error = nullptr);

    // Variant that includes the state parameter (recommended when called from the HTTP callback).
    bool HandleOAuthCallback(const std::string& code,
...
private:
    ...
    // OAuth (interactive) state for CSRF protection.
